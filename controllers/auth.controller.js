...
const authCtrl = {
    register: async (req, res) => {
        ...
        try {
            ...
            return res.json({
                status: true, message: "Registered successfully.", data: {
                    _id: saved_user._id,
                    username: saved_user.username
                }
            });
        } catch (error) {
            return res.status(400).json({ status: false, message: "Something went wrong.", data: error });
        }
    },
    login: async (req, res) => {
        const { username, password } = req.body;
    ...
    getToken: async (req, res) => {
        const user_id = req.userData.sub;
        const accessToken = jwt.sign({ sub: user_id }, process.env.JWT_ACCESS_SECRET, { expiresIn: process.env.JWT_ACCESS_TIME });
